	...
	case ".xlsx":
		...
		for i := 1; i < len(rows); i++ {
			migration := models.MigrationConfig{
				...
				SourceURL:              rows[i][2],
				TargetURL:              rows[i][3],
				SourceToken:            rows[i][4],
				TargetToken:            rows[i][5],
				KeepAllBranchesHistory: parseBool(rows[i][6]),
				DesiredGitWorkflow:     rows[i][7],
				DeleteSourceRepo:       parseBool(rows[i][8]),
				CarryUsers:             parseBool(rows[i][9]),
				SyncTeams:              parseBool(rows[i][10]),
				InviteMissingUsers:     parseBool(rows[i][11]),
				TransferIssues:         parseBool(rows[i][12]),
				TransferWiki:           parseBool(rows[i][13]),
				TransferPullRequests:   parseBool(rows[i][14]),
				TransferCiCdPipelines:  parseBool(rows[i][15]),
			}
